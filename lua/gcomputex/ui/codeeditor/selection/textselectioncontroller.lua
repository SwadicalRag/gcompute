local self = {}
GCompute.CodeEditor.TextSelectionController = GCompute.MakeConstructor (self)
...
function self:ctor (codeEditor, textSelection)
	...
	self.InitialSelectionStart = GCompute.CodeEditor.LineColumnLocation ()
	self.InitialSelectionEnd   = GCompute.CodeEditor.LineColumnLocation ()
				...
				if clickedOnLineNumbers then
					...
					elseif SysTime () - self.LastLeftMouseDownTime < 0.4 then
						...
						if not shift or alt then
							self.Selection:SetSelectionMode (alt and GCompute.CodeEditor.SelectionMode.Block or GCompute.CodeEditor.SelectionMode.Regular)
...
function self:ConvertToRegularSelection (preferredLine)
	if self.Selection:GetSelectionMode () == GCompute.CodeEditor.SelectionMode.Block then
		self.Selection:SetSelectionMode (GCompute.CodeEditor.SelectionMode.Regular)
...
function self:SelectLine (line, modifySelection)
	local spanStart = GCompute.CodeEditor.LineColumnLocation (line, 0)
	local spanEnd
	if self.Editor.Document:GetLine (line + 1) then
		spanEnd = GCompute.CodeEditor.LineColumnLocation (line + 1, 0)
	else
		spanEnd = GCompute.CodeEditor.LineColumnLocation (line, self.Editor.Document:GetLine (line):GetColumnCount (self.Editor:GetTextRenderer ()))
	end
...
function self:SelectWord (lineColumnLocation, modifySelection)
	...
	if lineColumnLocation:GetColumn () > 0 and lineColumnLocation:GetColumn () >= line:GetColumnCount (self.Editor.TextRenderer) then
		location = self.Editor:GetDocument ():ColumnToCharacter (GCompute.CodeEditor.LineColumnLocation (lineColumnLocation:GetLine (), line:GetColumnCount (self.Editor.TextRenderer) - 1), self.Editor:GetTextRenderer ())
	else
	...
	self:SelectSpan (
		GCompute.CodeEditor.LineColumnLocation (location:GetLine (), line:CharacterToColumn (leftCharacter,  self.Editor:GetTextRenderer ())),
		GCompute.CodeEditor.LineColumnLocation (location:GetLine (), line:CharacterToColumn (rightCharacter, self.Editor:GetTextRenderer ())),
		modifySelection
	)
end